...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        # patch_embeds: [batch, embed_dim, num_patches_H, num_patches_W]
        patch_embeds = self.patch_embedding(pixel_values)
        # embeddings: [batch, embed_dim, num_patches]
...
class SiglipAttention(nn.Module):
    def __init__(self, config: SiglipVisionConfig):
        ...
        self.scale = self.head_dim ** -0.5 # Equal to 1 / sqrt(self.head_dim)
...
class SiglipVisionModel(nn.Module):
    ...
    def forward(self, pixel_values) -> Tuple:
        # [batch, channels, height, width] -> [batch, num_patches, embd_dim]
